...
// Source data GeoJSON
const DATA_URL =
  './data/geo-processed.json'; // eslint-disable-line
...
function getTooltip({object}) {
  ...
  <div>${object.properties.ID_LAW} / parcel</div>
...
export default function App({data = DATA_URL, mapStyle = MAP_STYLE}) {
  ...
  const layers = [
    ...
    new GeoJsonLayer({
      ...
      extruded: true,
      wireframe: false,
      getElevation: f => Math.random() * 1000000,
      getFillColor: f => [Math.random()*255,Math.random()*255,Math.random()*255, 255],
